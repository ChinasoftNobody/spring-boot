...
import com.lgh.spring.boot.pojo.login.LoginStatusQuery;
...
public class LoginController {
    ...
    @ApiOperation(value = "用户ticket验证",notes = "用户ticket验证")
    public Response loginStatus(@RequestBody LoginStatusQuery query){
        if(query == null || StringUtils.isEmpty(query.getSso_ticket())){
            return ResponseUtil.failed("sso_ticket not fount");
        }
        return loginService.loginStatus(query.getSso_ticket());
    }
}
